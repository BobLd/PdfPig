...
{
    public sealed class JpegColorConverter
    {
        public static readonly JpegColorConverter Shared  = new JpegColorConverter();

        protected const int ClampTableOffset = 256;
        private const int Shift = 16;
        private const int OneHalf = 1 << (Shift - 1);

        protected readonly byte[] _clampTable;
        protected readonly int[] _crRTable;
        protected readonly int[] _cbBTable;
        protected readonly int[] _crGTable;
        protected readonly int[] _cbGTable;
        protected readonly int[] _yTable;
        ...
        */
        ...
        public void ConvertYCbCrK8ToCmyk24(ReadOnlySpan<byte> ycbcr, Span<byte> rgb, int count)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                cb = Unsafe.Add(ref sourceRef, 1);
                cr = Unsafe.Add(ref sourceRef, 2);
                k = Unsafe.Add(ref sourceRef, 3);

                yTableValue = yTable[y];
                destinationRef = (byte)(255 - clampTable[ClampTableOffset + yTableValue + _crRTable[cr]]);
                Unsafe.Add(ref destinationRef, 1) = (byte)(255 - clampTable[ClampTableOffset + yTableValue + ((_cbGTable[cb] + _crGTable[cr]) >> Shift)]);
                Unsafe.Add(ref destinationRef, 2) = (byte)(255 - clampTable[ClampTableOffset + yTableValue + _cbBTable[cb]]);
                Unsafe.Add(ref destinationRef, 3) = k;
        ...
        public void ConvertCmykToRgb(ReadOnlySpan<byte> cmyk, Span<byte> rgb, int count)
        {
            ...
            ref byte sourceRef = ref MemoryMarshal.GetReference(cmyk);
            ref byte destinationRef = ref MemoryMarshal.GetReference(rgb);
            ...
            {
                c = sourceRef;
                m = Unsafe.Add(ref sourceRef, 1);
                y = Unsafe.Add(ref sourceRef, 2);
                k = Unsafe.Add(ref sourceRef, 3);

                double kFact = 1.0 - k / 255.0;

                destinationRef = Convert.ToByte(255.0 * (1 - c / 255.0) * kFact);
                Unsafe.Add(ref destinationRef, 1) = Convert.ToByte(255.0 * (1 - m / 255.0) * kFact);
                Unsafe.Add(ref destinationRef, 2) = Convert.ToByte(255.0 * (1 - y / 255.0) * kFact);

namespace UglyToad.PdfPig.Functions
{
    ...
    internal sealed class PdfFunctionType3 : PdfFunction
    {
            ...
            : base(function, domain, range)
        {
            ...
            Bounds = bounds.Data.OfType<NumericToken>().Select(t => t.Double).ToArray();
            Encode = encode.Data.OfType<NumericToken>().Select(t => t.Double).ToArray();
        }
